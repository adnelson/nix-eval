...
data Native v
  = NativeValue (Result v)
  -- ^ A terminal value (or an error)
  | NativeFunction (Result v -> Result (Native v))
  -- ^ A function which lets us take the "next step" given a value.
  -- The input is lazy, so it's a `Result`. Similarly, the result of
  -- the function might be an error, and so is a 'result'.
  deriving (Generic)
...
instance (Natify t, EvalResult t ~ WHNFValue)
         => Natify (LazyValue -> t) where
  type EvalResult (LazyValue -> t) = EvalResult t
  natify :: (LazyValue -> t) -> Native (EvalResult t)
...
instance (Natify t, EvalResult t ~ WHNFValue)
  ...
  type EvalResult (WHNFValue -> t) = EvalResult t
  natify function = NativeFunction $ \lazy -> do
